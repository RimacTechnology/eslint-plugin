...
function lintModuleVariablesNewline(node, context, moduleType) {
    // If there is only one import/export leave it as is
    ...
    for (let i = 1; i < moduleVariables.length; i++) {
        ...
        if (moduleVariables[i].loc.start.line === moduleVariables[i - 1].loc.start.line) {
            ...
            const namedImportAfterDefault =
                moduleType === IMPORT && node.specifiers[i].type === 'ImportSpecifier' &&
                node.specifiers[i - 1] && node.specifiers[i - 1].type === 'ImportDefaultSpecifier'
            ...
            if (namedImportAfterDefault) {
                ...
                report((fixer) => 
                    fixer.replaceTextRange(rangeAfterBrace, '{\n'),
                )
            } else {
